...
public enum Ball {
	...
	private static final Map<Byte, Ball> ballMapping = new HashMap<Byte, Ball>();
	
	// Build the mapping.
	static {
		for(Ball ball : Ball.values()) {
			ballMapping.put(ball.getValue(), ball);
		}
	}
	
	/**
	 * Returns the <code>Ball</code> that has the same identifier as given by value.
	 * If no matching <code>Ball</code> entry is found, an <code>IllegalArgumentException</code> is thrown.
	 * @param value The unique identifier of the <code>Ball</code> entry to return.
	 */
	public static Ball fromByte(byte value) {
		if(!ballMapping.containsKey(value)) {
			throw new IllegalArgumentException("Unkown ball type " + value);
		}
		
		return ballMapping.get(value);
	}
}
